...
function addElement() {
  ...
  const packetName = elements.nameInput.val().split(' ').map((element) => {
    return el.charAt(0).toUpperCase() + element.slice(1);
  }).join(' ');
...
function moduleInit() {
  ...
  sql.connect(databasePath, key, 'aes-256-ctr');
  sql.runAsync(
    ...
    buttons: {
      Удалить: function () {
        if (confirm('Вы действительно хотите удалить данную позицию?')) {
          ...
          rebuildTable();
          $('#scoreOutput').html(documents.packetsList.length);
  ...
  elements.nameInput.autocomplete({
    source: (request, response) => {
      const results = $.ui.autocomplete.filter(availableNames, request.term);
